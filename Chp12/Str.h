...
public:
	...
	//null로 끝나는 char 차입의 배열에서 Str객체를 생성
	Str(const char* cp) {
		//메모리 할당
	...
	const iterator begin() const { return value; }

	//end()
	iterator end() { return value + length; }
	const iterator end() const { return value + length; }
...
Str operator+(Str&, const char*);	//Q5
